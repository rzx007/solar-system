...
function focusBounds(bd){
  ...
  return { lo:Math.max(rr*1.30,0.35), hi:Math.max(rr*45,140) };
}
function clearFocus(){
  ctrl.focus=null;
...
function tick(){
  ...
  /* 相机：使用指数阻尼，保证低帧率下跟随手感一致 */
  if(ctrl.focus){
    ctrl.focus.tilt.getWorldPosition(tmpPos);
    ctrl.want.lerp(tmpPos,1-Math.exp(-14.0*dt));
  }
  ...
  /* 聚焦时按天体半径约束缩放区间，避免固定值导致巨人行星贴脸 */
  if(ctrl.focus){
    var fr=ctrl.focus.cfg.r;
    var lo=Math.max(fr*2.0,0.9);
    if(ctrl.dDist<lo){ ctrl.dDist=lo; }
    var hi=Math.max(fr*70,160);
    if(ctrl.dDist>hi){ ctrl.dDist=hi; }
  }
